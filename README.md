...
- **アンビエントモード**: オプトイン式。`@` なしの会話が落ち着いたタイミングでアシスタントが自発的に発言します。
- **参加者表示 / 状態表示**: 接続中メンバー、`考え中 / 入力中 / 順番待ち` を表示。
- **1チャンネル1体の直列化**: 同時投稿してもアシスタントのターンは1つずつ（順番待ち）。
- **Markdown 表示**: アシスタントの応答は Markdown でレンダリング。
- **チャンネル削除**: 一覧から個別に削除可能。
- **会話履歴の永続化（SQLite）**: チャンネルとメッセージを `.data/channels.db` に保存。サーバ再起動後も会話が残ります。
...
| `lib/store/` | 会話ストア（`ChannelStore` interface＋in-memory / SQLite 実装。`STORE_DRIVER` で切り替え） |
...
- **状態は SQLite またはメモリ上のオブジェクト**: 会話履歴はデフォルトで SQLite（`lib/store/sqlite.ts`）、または in-memory の `Map`（`lib/store/memory.ts`）。`STORE_DRIVER` 環境変数で切り替え可能。
...
- **再起動で状態消失（`memory` モード時のみ）**: `STORE_DRIVER=sqlite`（デフォルト）では `.data/channels.db` に永続化されるため再起動後も残ります。`memory` に切り替えた場合は揮発します。
...
- better-sqlite3（チャンネル・メッセージの永続化）
...
# 会話ストア: sqlite（永続化・推奨）/ memory（揮発・再起動で消える）
STORE_DRIVER=sqlite
# STORE_FILE=.data/channels.db  # 保存先（既定値）
```
...
| `STORE_DRIVER` | `memory` | `sqlite`＝永続化（推奨）、`memory`＝揮発（再起動で消える） |
| `STORE_FILE` | `.data/channels.db` | SQLite ドライバの保存先 |
...
- **`memory` モードでは再起動で消失**: `STORE_DRIVER=memory` に設定した場合、会話はプロセス内にのみ保持されます。デフォルトの `sqlite` では永続化されます。
